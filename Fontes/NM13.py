...
# ✅ Função auxiliar para verificar se a unidade está disponível
def unidade_disponivel():
    return os.path.exists(origem) and os.path.isdir(origem)
...
class FolderSelectorWindow(QMainWindow):
    def __init__(self):
        ...
        # ✅ Verifica se a unidade está acessível antes de tentar listar pastas
        if unidade_disponivel():
            self.selecionar_pastas()
        else:
            self.selecionar_servidor()

    def selecionar_servidor(self):
        self.selecionarServidor = SetServer()
        self.selecionarServidor.show()

        # ✅ Após mostrar janela de seleção, verifica se a unidade está disponível
        if unidade_disponivel():
            self.selecionar_pastas()
