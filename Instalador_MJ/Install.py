...
resources_dir = os.path.join(base_dir, 'Zips')
...
# Desktop público (para todos os usuários)
...
def create_scheduled_task(task_name, executable_path):
    ...
    # Cria a nova tarefa com conta SYSTEM
    create_cmd = [
        'schtasks', '/Create',
        '/SC', 'ONLOGON',
        '/RL', 'HIGHEST',
...
# 3. Criar atalhos no Desktop Público
vhdx_exe = os.path.join(vhdxrun_dest, 'VHDXRun.exe')
network_setup_exe = os.path.join(mj_network_dest, 'NetworkSetup.exe')

shortcut1 = os.path.join(public_desktop, 'Servidor - Conectar.lnk')
shortcut2 = os.path.join(public_desktop, 'Servidor - Configuração.lnk')

create_shortcut(vhdx_exe, shortcut1)
create_shortcut(network_setup_exe, shortcut2)
